...
function sanitizeText(text) {
  ...
  // Remove common prompt injection patterns - إزالة أنماط الحقن الشائعة
  return text
    .replace(/ignore\s+(all\s+)?(previous|above|prior)\s+(instructions?|text|rules?)/gi, '')
    .replace(/system\s*:/gi, '')
...
function buildPrompt(text, targetLang) {
  ...
  const sanitizedLang = String(targetLang).replace(/[^a-zA-Z\s]/g, '').trim();
  
  return `You are a professional manga/manhwa translator. Translate the following text to ${sanitizedLang}.
